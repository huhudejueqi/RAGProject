...
@dataclass(frozen=True)
class SourceMatch:
    """Ranked source match produced from scenario source_patterns.

    调用顺序：场景解析入口 -> SourceMatch。
    """
    ...
    def as_dict(self) -> dict[str, str | int | float]:
        """Return JSON-serializable match diagnostics.
...
def score_source_map(query: str, scenario: ScenarioDefinition) -> dict[str, int]:
    ...
    """
    # 归一化处理：保留原始 query 处理大小写敏感匹配，同时用 lower 版处理不敏感场景
    normalized = query.strip().lower()
    scores: dict[str, int] = {}
    for index, (source, pattern) in enumerate(scenario.compiled_source_patterns().items()):
        # 同时在原文本和归一化文本上做匹配，避免因用户意外大写/全角字符丢失命中
        matches = list(pattern.finditer(query)) + list(pattern.finditer(normalized))
        if not matches:
            continue
        # 计分公式：命中次数 * 10 + 匹配文本总长度 - source 顺序索引
        # 10 倍放大确保多次命中优先于单次长命；减索引让 valid_sources 靠前的 source 有小幅优先级优势
...
def rank_source_matches(query: str, scenario: ScenarioDefinition) -> tuple[SourceMatch, ...]:
    """Return source matches sorted by score with stable confidence labels.

    ``confidence`` is a normalized diagnostic value, not a model probability.
    It helps trace panels explain whether the best source clearly outranked the
    other candidates.
...
def detect_scenario_boundary(query: str, current_scenario: ScenarioDefinition) -> ScenarioBoundaryDecision:
    ...
    返回：
        场景边界检测结果。
    """
    # 跨场景检测的核心逻辑：先看当前场景是否足够可信，再遍历其他场景寻找强匹配
