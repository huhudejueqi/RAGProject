...
class QueryServiceContext:
    ...
    调用顺序：FastAPI 路由层 -> QueryServiceContext。
    """
    ...
    def from_debug_request(
        ...
        *,
        ...
        调用顺序：FastAPI 路由层 -> QueryServiceContext.from_debug_request()。
        """
        return cls(
    ...
    def from_ws_payload(cls, payload: dict[str, Any]) -> "QueryServiceContext":
        ...
        调用顺序：FastAPI 路由层 -> QueryServiceContext.from_ws_payload()。
        """
        return cls(
    ...
    def service_args(self) -> tuple[Any, ...]:
        ...
        调用顺序：FastAPI 路由层 -> QueryServiceContext.service_args()。
        """
        return (
            self.query,
            self.source_filter,
